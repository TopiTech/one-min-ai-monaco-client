...
> [!NOTE]
> 本アプリケーションのバックエンド（[api-client.js](file:///c:/Users/mibu0/Documents/develop/one-min-ai-monaco-client/utils/api-client.js)）では、環境変数 `ONE_MIN_API_KEY` を用いて、自動的にリクエストヘッダーを設定するよう実装されています。
...
AI Feature API (`/api/features`) や Chat API の Non-Streaming 時には、以下の構造を持つ JSON レスポンスオブジェクトが返却されます。
...
```json
{
  "aiRecord": {
    ...
    "input": "...",
    "output": [
      "https://asset.1min.ai/outputs/generated-image-url.png"
    ],
    "inputToken": 0,
    "outputToken": 0,
    "totalToken": 0,
...
- **`input`**: AIに送信された最終入力コンテキストのプレーンテキスト。
- **`output`**: AIによって生成された出力。
  - テキスト系 (`UNIFY_CHAT_WITH_AI`, `CODE_GENERATOR`) の場合: `output` は配列ではなく単一のテキスト文字列、もしくは配列の中に返却テキストが格納されます。
  - 画像系 (`IMAGE_GENERATOR`, `IMAGE_EDITOR`) の場合: 生成された画像ファイルの永続URLの配列になります。
- **`credit`**: 本リクエストの処理によって消費されたクレジット数。
- **`status`**: リクエストの処理状態（例: `"COMPLETED"`, `"FAILED"`）。
