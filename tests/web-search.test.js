...
describe('web-search payload helpers', () => {
    describe('parseWebSearchParams', () => {
        ...
        test('should reject invalid numOfSite', () => {
            expect(() => parseWebSearchParams({ numOfSite: 0 })).toThrow('numOfSite must be a number between 1 and 10');
        });

        test('should reject invalid maxWord', () => {
            expect(() => parseWebSearchParams({ maxWord: 50 })).toThrow('maxWord must be a number between 100 and 10000');
        });
    });

    describe('buildWebSearchSettings', () => {
        test('should include optional limits only when provided', () => {
            expect(buildWebSearchSettings({
                webSearch: true,
                parsedNumOfSite: 3,
            })).toEqual({
                webSearch: true,
